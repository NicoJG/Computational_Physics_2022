...
t, E_pot, E_kin, T, P, a0 = np.genfromtxt("data/H1_4_temp_scaling.csv", delimiter=",", unpack=True)
t2, E_pot2, E_kin2, T2, P2, a02 = np.genfromtxt("data/H1_4_pressure_scaling.csv", delimiter=",", unpack=True)
t3, E_pot3, E_kin3, T3, P3, a03 = np.genfromtxt("data/H1_4_temp_decreasing.csv", delimiter=",", unpack=True)
...
# get header metadata
with open("data/H1_4_temp_scaling.csv", "r") as file:
    metadata_str = "".join([file.readline() for i in range(4)])
    metadata_str = metadata_str.replace("# ","")
    metadata = json.loads(metadata_str)
    
T_melting = metadata["T_desired"]
...
t_switch_on_T_decreasing = (t2[-1]+t3[0])/2
...
plt.axvline(t_switch_on_T_decreasing, linestyle="--", color="k", alpha=0.5)
plt.plot(t, T, "C0")
plt.plot(t2, T2, "C0")
plt.plot(t3, T3, "C0")
plt.axhline(T_desired, linestyle=":", color="k", label=f"desired $T={T_desired:.2f} \\: \\mathrm{{K}}$")
plt.axhline(T_melting, linestyle=":", color="k", label=f"melting $T={T_melting:.2f} \\: \\mathrm{{K}}$")
plt.xlabel("time [ps]")
...
plt.plot(t3, E_tot3, "C2")
plt.axhline(E_tot3[-1], linestyle=":", color="k", label=f"end $E_\mathrm{{tot}}={E_tot3[-1]:.2f} \\: \\mathrm{{eV}}$")
plt.xlabel("time [ps]")
...
plt.plot(t2, a02, "C3")
plt.plot(t3, a03, "C3")
plt.axhline(a03[-1], linestyle=":", color="k", label=f"end $a_0={a03[-1]:.4f} \\: \\mathrm{{Å}}$")
plt.xlabel("time [ps]")
plt.ylabel("lattice constant [Å]")
plt.legend(loc="lower right")

plt.tight_layout()
plt.savefig("plots/H1_4_equilibration.pdf")
...
# get header metadata
with open("data/H1_4_after_scaling.csv", "r") as file:
    metadata_str = "".join([file.readline() for i in range(4)])
    metadata_str = metadata_str.replace("# ","")
    metadata = json.loads(metadata_str)
...
plt.tight_layout()
plt.savefig("plots/H1_4_positions.pdf")
...
plt.sca(axs[2])
plt.plot(t, E_tot, "C2")
plt.axhline(E_tot3[-1], linestyle=":", alpha=0.5, color="k", label=f"end $E_\mathrm{{tot}}={E_tot3[-1]:.2f} \\: \\mathrm{{eV}}$")
...
plt.sca(axs[3])
plt.plot(t, a0, "C3")
plt.axhline(a03[-1], linestyle=":", alpha=0.5, color="k", label=f"end $a_0={a03[-1]:.4f} \\: \\mathrm{{Å}}$")
...
plt.tight_layout()
plt.savefig("plots/H1_4_after_equilibration.pdf")